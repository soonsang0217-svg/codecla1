...
2. **동작 추가** → **"건강 샘플 찾기"**(Find Health Samples) 액션을 두 개 추가:
   - 유형: **심박수**, 시작일: **지난 1일**, 정렬/제한은 기본값 그대로(전부 가져오기)
   - 유형: **심박 변이**, 시작일: **지난 1일**, 마찬가지로 전부 가져오기
3. **"URL의 콘텐츠 가져오기"**(Get Contents of URL) 액션 추가:
   - URL: `https://<프로젝트명>.vercel.app/api/health/ingest`
   - 방법: **POST**
   - 헤더: `Authorization: Bearer <HEALTH_INGEST_TOKEN 값>`, `Content-Type: application/json`
   - 요청 본문: **JSON** 선택 → 필드 2개 추가:
     - 키 `heartRateRaw`, 형식은 **텍스트**로 둔 채 값 칸을 눌러 **심박수 건강 샘플** 매직 변수 선택
     - 키 `heartRateVariabilityRaw`, 마찬가지로 값 칸에 **심박 변이 건강 샘플** 매직 변수 선택
   - (수면 시간/점수는 아직 자동화하지 않음 — 원하면 `sleepDurationMinutes`/`sleepScore` 키에
     숫자를 직접 넣어 수동으로 보낼 수 있습니다)

값 칸에 매직 변수(목록)를 넣으면, 단축어가 자동으로 값들을 줄바꿈으로 이어붙인 텍스트로
바꿔 보냅니다 (예: `"72\n68\n75\n..."`) — 서버가 이 형식을 그대로 파싱해서 오늘의 추이
그래프로 보여줍니다. 그날 해당 데이터가 없으면(예: 애플워치로 수면을 재지 않은 날) 빈
문자열이 오는데, 이 경우 이전 값을 그대로 유지합니다.
